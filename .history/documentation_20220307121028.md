...
Let as try another card once more to for a clearer understanding. Based on the information provided above, create a card component called `Card2` inside the `index` directory using `forwardRef` like 